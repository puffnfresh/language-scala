package scalaz {
  ...
  sealed abstract class LensFamily[A1, A2, B1, B2]  {
    ...
    , C2 ] =
      andThen(that)
    def sum[C1, C2]( that : => LensFamily[ C1
    , C2
    , B1
    , B2 ] ) : LensFamily[(A1 \/ C1), (A2 \/ C2), B1, B2] =
      lensFamily( {
        case -\/ (a) =>
          ((run(a)) map \/.left)
        case \/- (c) =>
    ...
    , D2 ] ) : LensFamily[(A1, C1), (A2, C2), (B1, D1), (B2, D2)] =
      lensFamily({   case (a, c) =>   ((run(a)) *** that.run(c)) })
  ...
  trait LensFamilyFunctions  {
    ...
    def firstLensFamily[A1, A2, B] : LensFamily[(A1, B), (A2, B), A1, A2] =
      lensFamily({   case (a, b) =>   IndexedStore(((x) => (x, b)), a) })
    def secondLensFamily[A, B1, B2] : LensFamily[(A, B1), (A, B2), B1, B2] =
      lensFamily({   case (a, b) =>   IndexedStore(((x) => (a, x)), b) })
  ...
  trait LensFunctions  extends LensFamilyFunctions {
    ...
    def firstLens[A, B] : ((A, B) @> A) =
      lens({   case (a, b) =>   Store(((x) => (x, b)), a) })
    def secondLens[A, B] : ((A, B) @> B) =
      lens({   case (a, b) =>   Store(((x) => (a, x)), b) })
    ...
    def nelTailLens[A] : (NonEmptyList[A] @> List[A]) =
      lens( ((l) => Store( ((ll) => NonEmptyList.nel( l.head
      , IList.fromList(ll) ))
      , l.tail.toList )) )
  ...
  abstract class LensInstances  extends LensInstances0 {
    ...
    , Map[K, V] ] ) {
      def member(k : K) : LensFamily[S1, S2, Option[V], Option[V]] =
        ...
        , S2
        , Option[V]
        , Option[V] ]( ((s) => ((opt) => ((lens.mod( (( m : Map[ K
        , V ] ) => ((opt match {
          case Some (v) =>
            ((m) + ((k) -> v))
          case None =>
