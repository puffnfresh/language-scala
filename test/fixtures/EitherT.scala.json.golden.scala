...
object EitherT extends EitherTInstances {
  ...
  def eitherTU[FAB, AB, A0, B0](fab: FAB)( implicit u1: Unapply[Functor, FAB] {type A = AB}
...
private trait EitherTFunctor[F[_], E]  extends Functor[EitherT[E, F, *]] {
  ...
  override def map[A, B](fa: EitherT[E, F, A])(f: A => B): EitherT[E, F, B] =
    fa map f
}
...
private trait EitherTBind[F[_], E]  extends Bind[EitherT[E, F, *]] with EitherTFunctor[F, E] {
  ...
  final def bind[A, B](fa: EitherT[E, F, A])(f: A => EitherT[E, F, B]): EitherT[E, F, B] =
    fa flatMap f
}
...
private trait EitherTFoldable[F[_], E]  extends Foldable.FromFoldr[EitherT[E, F, *]] {
  ...
  override def foldRight[A, B](fa: EitherT[E, F, A], z: => B)(f: (A, => B) => B): B =
    fa.foldRight(z)(f)
}
...
private trait EitherTTraverse[F[_], E]  extends Traverse[EitherT[E, F, *]] with EitherTFoldable[F, E] {
  ...
  def traverseImpl[G[_]: Applicative, A, B](fa: EitherT[E, F, A])(f: A => G[B]): G[EitherT[E, F, B]] =
    fa traverse f
}
...
private trait EitherTBifunctor[F[_]]  extends Bifunctor[EitherT[*, F, *]] {
  ...
  override def bimap[A, B, C, D](fab: EitherT[A, F, B])(f: A => C, g: B => D): EitherT[C, F, D] =
    fab.bimap(f, g)
}
...
private trait EitherTBitraverse[F[_]]  extends Bitraverse[EitherT[*, F, *]] with EitherTBifunctor[F] with EitherTBifoldable[F] {
  ...
  def bitraverseImpl[G[_]: Applicative, A, B, C, D](fab: EitherT[A, F, B])(f: A => G[C], g: B => G[D]): G[EitherT[C, F, D]] =
    fab.bitraverse(f, g)
}
