package scalaz {
  ...
  trait Apply[F[_]]  extends Functor[F] { self =>
    ...
    , S ) ] )(implicit R : Reducer[A, B]) : Maybe[F[B]] =
      {
    ...
    def ap3[A, B, C, D](fa : => F[A], fb : => F[B], fc : => F[C])( f : F[ ( A
    , B
    , C ) => D ] ) : F[D] =
      ap(fc)(ap(fb)(ap(fa)(map(f)(_.curried))))
    def ap4[A, B, C, D, E]( fa : => F[A]
    , fb : => F[B]
    , fc : => F[C]
    , fd : => F[D] )(f : F[(A, B, C, D) => E]) : F[E] =
      ap(fd)(ap(fc)(ap(fb)(ap(fa)(map(f)(_.curried)))))
    def ap5[A, B, C, D, E, R]( fa : => F[A]
    , fb : => F[B]
    , fc : => F[C]
    , fd : => F[D]
    , fe : => F[E] )(f : F[(A, B, C, D, E) => R]) : F[R] =
      ap(fe)(ap(fd)(ap(fc)(ap(fb)(ap(fa)(map(f)(_.curried))))))
    def ap6[A, B, C, D, E, FF, R]( fa : => F[A]
    , fb : => F[B]
    , fc : => F[C]
    , fd : => F[D]
    , fe : => F[E]
    ...
    def apply3[A, B, C, D](fa : => F[A], fb : => F[B], fc : => F[C])( f : ( A
    , B
    , C ) => D ) : F[D] =
      ap(fc)(ap(fb)(map(fa)(f.curried)))
    def apply4[A, B, C, D, E]( fa : => F[A]
    , fb : => F[B]
    , fc : => F[C]
    , fd : => F[D] )(f : (A, B, C, D) => E) : F[E] =
      ap(fd)(ap(fc)(ap(fb)(map(fa)(f.curried))))
    def apply5[A, B, C, D, E, R]( fa : => F[A]
    , fb : => F[B]
    , fc : => F[C]
    , fd : => F[D]
    , fe : => F[E] )(f : (A, B, C, D, E) => R) : F[R] =
      ap(fe)(ap(fd)(ap(fc)(ap(fb)(map(fa)(f.curried)))))
    def apply6[A, B, C, D, E, FF, R]( fa : => F[A]
    , fb : => F[B]
    , fc : => F[C]
    , fd : => F[D]
    , fe : => F[E]
    ...
    def apply7[A, B, C, D, E, FF, G, R]( fa : => F[A]
    , fb : => F[B]
    , fc : => F[C]
    , fd : => F[D]
    , fe : => F[E]
    , ff : => F[FF]
    , fg : => F[G] )(f : (A, B, C, D, E, FF, G) => R) : F[R] =
    ...
    final def applying1[Z, A1](f : (A1) => Z)(implicit a1 : F[A1]) : F[Z] =
      map(a1)(f)
    final def applying2[Z, A1, A2](f : (A1, A2) => Z)( implicit a1 : F[A1]
    , a2 : F[A2] ) : F[Z] =
      apply2(a1, a2)(f)
    final def applying3[Z, A1, A2, A3]( f : ( A1
    , A2
    , A3 ) => Z )(implicit a1 : F[A1], a2 : F[A2], a3 : F[A3]) : F[Z] =
      apply3(a1, a2, a3)(f)
    final def applying4[Z, A1, A2, A3, A4]( f : ( A1
    , A2
    , A3
    , A4 ) => Z )( implicit a1 : F[A1]
    , a2 : F[A2]
    , a3 : F[A3]
    , a4 : F[A4] ) : F[Z] =
    ...
    def tuple2[A, B](fa : => F[A], fb : => F[B]) : F[(A, B)] =
      apply2(fa, fb)((_, _))
    def tuple3[A, B, C](fa : => F[A], fb : => F[B], fc : => F[C]) : F[ ( A
    , B
    , C ) ] =
      apply3(fa, fb, fc)((_, _, _))
    def tuple4[A, B, C, D]( fa : => F[A]
    , fb : => F[B]
    , fc : => F[C]
    , fd : => F[D] ) : F[(A, B, C, D)] =
      apply4(fa, fb, fc, fd)((_, _, _, _))
    def tuple5[A, B, C, D, E]( fa : => F[A]
    , fb : => F[B]
    , fc : => F[C]
    , fd : => F[D]
    , fe : => F[E] ) : F[(A, B, C, D, E)] =
    ...
    def lift3[A, B, C, D](f : (A, B, C) => D) : (F[A], F[B], F[C]) => F[D] =
      apply3(_, _, _)(f)
    def lift4[A, B, C, D, E](f : (A, B, C, D) => E) : ( F[A]
    , F[B]
    , F[C]
    , F[D] ) => F[E] =
      apply4(_, _, _, _)(f)
    def lift5[A, B, C, D, E, R](f : (A, B, C, D, E) => R) : ( F[A]
    , F[B]
    , F[C]
    , F[D]
    , F[E] ) => F[R] =
      apply5(_, _, _, _, _)(f)
    def lift6[A, B, C, D, E, FF, R](f : (A, B, C, D, E, FF) => R) : ( F[A]
    , F[B]
    , F[C]
    , F[D]
    , F[E]
    ...
    def lift7[A, B, C, D, E, FF, G, R](f : (A, B, C, D, E, FF, G) => R) : ( F[A]
    , F[B]
    , F[C]
    , F[D]
    , F[E]
    ...
    def lift8[A, B, C, D, E, FF, G, H, R]( f : ( A
    , B
    , C
    , D
    , E
    , FF
    , G
    , H ) => R ) : (F[A], F[B], F[C], F[D], F[E], F[FF], F[G], F[H]) => F[R] =
      apply8(_, _, _, _, _, _, _, _)(f)
    def lift9[A, B, C, D, E, FF, G, H, I, R]( f : ( A
    , B
    , C
    , D
    , E
    ...
    def lift10[A, B, C, D, E, FF, G, H, I, J, R]( f : ( A
    , B
    , C
    , D
    , E
    ...
    def lift11[A, B, C, D, E, FF, G, H, I, J, K, R]( f : ( A
    , B
    , C
    , D
    , E
    ...
    def lift12[A, B, C, D, E, FF, G, H, I, J, K, L, R]( f : ( A
    , B
    , C
    , D
    , E
    ...
    def liftReducer[A, B](implicit r : Reducer[A, B]) : Reducer[F[A], F[B]] =
      new Reducer[F[A], F[B]] {
        ...
        def snoc(fb : F[B], fa : F[A]) : F[B] =
          apply2(fb, fa)(r.snoc)
        override def unfoldrOpt[S](seed : S)( f : (S) => Maybe[ ( F[A]
        , S ) ] ) : Maybe[F[B]] =
          Apply.this.unfoldrOpt(seed)(f)
      }
    trait ApplyLaw  extends FunctorLaw {
      def composition[A, B, C]( fbc : F[(B) => C]
      , fab : F[(A) => B]
  ...
  trait IsomorphismApply[F[_], G[_]]  extends Apply[F]
   with IsomorphismFunctor[F, G] {
    ...
    override def apply2[A, B, C](fa : => F[A], fb : => F[B])( f : ( A
    , B ) => C ) : F[C] =
