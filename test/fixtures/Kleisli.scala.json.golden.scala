...
object Kleisli extends KleisliInstances {
  ...
  def iso[D[_], I, O[_]](instance: λ[a => I => O[a]] ~> D, decode: D ~> λ[a => I => O[a]]): D <~> Kleisli[O, I, *] =
...
private trait KleisliCompose[F[_]]  extends Compose[Kleisli[F, *, *]] {
  ...
  def compose[A, B, C](bc: Kleisli[F, B, C], ab: Kleisli[F, A, B]): Kleisli[F, A, C] =
    ab >=> bc
}

private trait KleisliArrow[F[_]]  extends Arrow[Kleisli[F, *, *]] with Choice[Kleisli[F, *, *]] with KleisliCompose[F] with KleisliProChoice[F] {
  implicit def F: Monad[F]
