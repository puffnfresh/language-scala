package scalaz {
  ...
  final case class Kleisli[M[_], A, B] (run : (A) => M[B]) { self =>
    ...
    def <=<[C](k : Kleisli[M, C, A])(implicit b : Bind[M]) : Kleisli[M, C, B] =
      ((k) >=> this)
    def compose[C](k : Kleisli[M, C, A])(implicit b : Bind[M]) : Kleisli[ M
    , C
    , B ] =
      ((k) >=> this)
    ...
    def flatMap[C]( f : (B) => Kleisli[ M
    , A
    , C ] )(implicit M : Bind[M]) : Kleisli[M, A, C] =
    ...
    import Liskov.{_}
    def unlift[N[_], FF[_]]( implicit M : Comonad[N]
    , ev : (this.type <~< Kleisli[λ[(α) => N[FF[α]]], A, B]) ) : Kleisli[ FF
    , A
    , B ] =
      kleisli[FF, A, B]( {
        ((a) => Comonad[N].copoint(((ev(self)) run a)))
      } )
    def unliftId[N[_]]( implicit M : Comonad[N]
    , ev : (this.type <~< Kleisli[N[*], A, B]) ) : Reader[A, B] =
      unlift[N, Id]
    def rwst[W, S]( implicit M : Functor[M]
    , W : Monoid[W] ) : ReaderWriterStateT[A, W, S, M, B] =
      ReaderWriterStateT( ((r, s) => M.map(self(r))( {
        ((b) => (W.zero, b, s))
      } )) )
    ...
    def endo(implicit M : Functor[M], ev : (A >~> B)) : Endomorphic[ Kleisli[ M
    , *
    , * ]
    , A ] =
  ...
  sealed abstract class KleisliInstances15  {
    implicit def kleisliDivisible[ F[_]
    , R ](implicit F0 : Divisible[F]) : Divisible[Kleisli[F, R, *]] =
  ...
  sealed abstract class KleisliInstances14  extends KleisliInstances15 {
    implicit def kleisliDecidable[ F[_]
    , R ](implicit F0 : Decidable[F]) : Decidable[Kleisli[F, R, *]] =
  ...
  sealed abstract class KleisliInstances13  extends KleisliInstances14 {
    implicit def kleisliFunctor[ F[_]
    , R ](implicit F0 : Functor[F]) : Functor[Kleisli[F, R, *]] =
  ...
  sealed abstract class KleisliInstances12  extends KleisliInstances13 {
    implicit def kleisliApply[ F[_]
    , R ](implicit F0 : Apply[F]) : Apply[Kleisli[F, R, *]] =
  ...
  sealed abstract class KleisliInstances11  extends KleisliInstances12 {
    implicit def kleisliDistributive[ F[_]
    , R ](implicit F0 : Distributive[F]) : Distributive[Kleisli[F, R, *]] =
  ...
  sealed abstract class KleisliInstances10  extends KleisliInstances11 {
    implicit def kleisliBind[F[_], R](implicit F0 : Bind[F]) : Bind[ Kleisli[ F
    , R
    , * ] ] =
      new KleisliBind[F, R] {
        def F =
  ...
  sealed abstract class KleisliInstances9  extends KleisliInstances10 {
    implicit def kleisliZip[F[_], R](implicit F : Zip[F]) : Zip[ Kleisli[ F
    , R
    , * ] ] =
      new Zip[Kleisli[F, R, *]] {
  ...
  sealed abstract class KleisliInstances8  extends KleisliInstances9 {
    implicit def kleisliApplicative[ F[_]
    , R ](implicit F0 : Applicative[F]) : Applicative[Kleisli[F, R, *]] =
      new KleisliApplicative[F, R] {
        implicit def F : Applicative[F] =
          F0
      }
    implicit def kleisliPlus[F[_], A](implicit F0 : Plus[F]) : Plus[ Kleisli[ F
    , A
    , * ] ] =
      new KleisliPlus[F, A] {
        implicit def F =
  ...
  sealed abstract class KleisliInstances7  extends KleisliInstances8 {
    implicit def kleisliBindRec[ F[_]
    , R ](implicit F0 : BindRec[F]) : BindRec[Kleisli[F, R, *]] =
      new KleisliBindRec[F, R] {
        implicit def F : BindRec[F] =
          F0
      }
    implicit def kleisliPlusEmpty[ F[_]
    , A ](implicit F0 : PlusEmpty[F]) : PlusEmpty[Kleisli[F, A, *]] =
  ...
  sealed abstract class KleisliInstances6  extends KleisliInstances7 {
    implicit def kleisliApplicativePlus[ F[_]
    , R ](implicit F0 : ApplicativePlus[F]) : ApplicativePlus[ Kleisli[ F
    , R
    , * ] ] =
    ...
    implicit def kleisliSemigroup[ F[_]
    , A
    , B ](implicit FB0 : Semigroup[F[B]]) : Semigroup[Kleisli[F, A, B]] =
  ...
  sealed abstract class KleisliInstances5  extends KleisliInstances6 {
    implicit def kleisliMonadError[F[_], E, R]( implicit F0 : MonadError[ F
    , E ] ) : MonadError[Kleisli[F, R, *], E] =
  ...
  sealed abstract class KleisliInstances4  extends KleisliInstances5 {
    implicit def kleisliMonadPlus[ F[_]
    , A ](implicit F0 : MonadPlus[F]) : MonadPlus[Kleisli[F, A, *]] =
  ...
  sealed abstract class KleisliInstances3  extends KleisliInstances4 {
    implicit def kleisliMonadReader[ F[_]
    , R ](implicit F0 : Monad[F]) : MonadReader[Kleisli[F, R, *], R] =
  ...
  abstract class KleisliInstances  extends KleisliInstances0 {
    implicit def kleisliArrow[F[_]](implicit F0 : Monad[F]) : Arrow[ Kleisli[ F
    , *
    ...
    implicit def kleisliContravariant[F[_], A] : Contravariant[ Kleisli[ F
    , *
    , A ] ] =
    ...
    implicit def kleisliMonoid[ F[_]
    , A
    , B ](implicit FB0 : Monoid[F[B]]) : Monoid[Kleisli[F, A, B]] =
      new KleisliMonoid[F, A, B] {
        implicit def FB =
          FB0
      }
    implicit def kleisliMonadTrans[R] : Hoist[ ( {type l[α[_], β] = Kleisli[ α
    , R
    , β ]})#l ] =
      new KleisliHoist[R] {
      }
  ...
  object Kleisli extends KleisliInstances {
    def kleisli[M[_], A, B](f : (A) => M[B]) : Kleisli[M, A, B] =
      Kleisli(f)
    def kleisliU[A, MB](f : (A) => MB)( implicit MB : Unapply[ Bind
    , MB ] ) : Kleisli[MB.M, A, MB.A] =
    ...
    def local[M[_], A, R](f : (R) => R)(fa : Kleisli[M, R, A]) : Kleisli[ M
    , R
    , A ] =
      ((fa) local f)
    import Isomorphism.{<~>, IsoFunctorTemplate}
    def iso[D[_], I, O[_]]( instance : (λ[(a) => (I) => O[a]] ~> D)
    , decode : (D ~> λ[(a) => (I) => O[a]]) ) : (D <~> Kleisli[O, I, *]) =
      new IsoFunctorTemplate[D, Kleisli[O, I, *]] {
        def from_[A](fa : Kleisli[O, I, A]) : D[A] =
          instance(fa.run)
        def to_[A](fa : D[A]) : Kleisli[O, I, A] =
          Kleisli[O, I, A](decode(fa))
      }
  }
  ...
  private trait KleisliDivisible[F[_], R]  extends Divisible[Kleisli[F, R, *]] {
    implicit def F: Divisible[F]
    override def conquer[Z] : Kleisli[F, R, Z] =
      Kleisli(((_) => F.conquer))
    override def divide2[A1, A2, Z]( a1 : => Kleisli[F, R, A1]
    , a2 : => Kleisli[F, R, A2] )(f : (Z) => (A1, A2)) : Kleisli[F, R, Z] =
      Kleisli(((r) => F.divide2(a1.run(r), a2.run(r))(f)))
  }

  private trait KleisliDecidable[F[_], R]  extends Decidable[Kleisli[F, R, *]]
   with KleisliDivisible[F, R] {
    implicit def F: Decidable[F]
    override def choose2[Z, A1, A2]( a1 : => Kleisli[F, R, A1]
    , a2 : => Kleisli[F, R, A2] )(f : (Z) => (A1 \/ A2)) : Kleisli[F, R, Z] =
  ...
  private trait KleisliFunctor[F[_], R]  extends Functor[Kleisli[F, R, *]] {
    implicit def F: Functor[F]
    override def map[A, B](fa : Kleisli[F, R, A])(f : (A) => B) : Kleisli[ F
    , R
    , B ] =
      ((fa) map f)
  }

  private trait KleisliApply[F[_], R]  extends Apply[Kleisli[F, R, *]]
   with KleisliFunctor[F, R] {
    implicit def F: Apply[F]
    override def ap[A, B](fa : => Kleisli[F, R, A])( f : => Kleisli[ F
    , R
    , (A) => B ] ) : Kleisli[F, R, B] =
      Kleisli[F, R, B](((r) => F.ap(fa(r))(f(r))))
  }

  private trait KleisliDistributive[F[_], R]  extends Distributive[ Kleisli[ F
  , R
  , * ] ]
   with KleisliFunctor[F, R] {
    implicit def F: Distributive[F]
    override def distributeImpl[ G[_]: Functor
    , A
    , B ](a : G[A])(f : (A) => Kleisli[F, R, B]) : Kleisli[F, R, G[B]] =
      Kleisli(((r) => F.distribute(a)(((f(_)) run r))))
  }

  private trait KleisliBind[F[_], R]  extends Bind[Kleisli[F, R, *]]
   with KleisliApply[F, R] {
    implicit def F: Bind[F]
    override final def bind[A, B](fa : Kleisli[F, R, A])( f : (A) => Kleisli[ F
    , R
    , B ] ) =
      ((fa) flatMap f)
  }

  private trait KleisliApplicative[F[_], R]  extends Applicative[ Kleisli[ F
  , R
  , * ] ]
   with KleisliApply[F, R] {
    implicit def F: Applicative[F]
  ...
  private trait KleisliBindRec[F[_], R]  extends BindRec[Kleisli[F, R, *]]
   with KleisliBind[F, R] {
    implicit def F: BindRec[F]
    def tailrecM[A, B](a : A)(f : (A) => Kleisli[F, R, (A \/ B)]) : Kleisli[ F
    , R
    , B ] =
      Kleisli(((r) => F.tailrecM(a)(f(_).run(r))))
  }

  private trait KleisliMonad[F[_], R]  extends Monad[Kleisli[F, R, *]]
   with KleisliApplicative[F, R]
   with KleisliBind[F, R] {
    implicit def F: Monad[F]
  }

  private trait KleisliMonadReader[F[_], R]  extends MonadReader[ Kleisli[ F
  , R
  , * ]
  , R ]
   with KleisliApplicative[F, R]
  ...
  private trait KleisliHoist[R]  extends Hoist[ ( {type l[α[_], β] = Kleisli[ α
  , R
  , β ]})#l ] {
    def hoist[M[_]: Monad, N[_]](f : (M ~> N)) : (Kleisli[M, R, *] ~> Kleisli[ N
    , R
    , * ]) =
  ...
  private trait KleisliMonadPlus[F[_], R]  extends MonadPlus[Kleisli[F, R, *]]
   with KleisliPlusEmpty[F, R]
   with KleisliMonad[F, R] {
    implicit def F: MonadPlus[F]
  }

  private trait KleisliMonadError[F[_], E, R]  extends MonadError[ Kleisli[ F
  , R
  , * ]
  , E ]
   with KleisliMonad[F, R] {
    implicit def F: MonadError[F, E]
    def handleError[A](fa : Kleisli[F, R, A])( f : (E) => Kleisli[ F
    , R
    , A ] ) : Kleisli[F, R, A] =
      Kleisli.kleisli[ F
      , R
      , A ](((r) => F.handleError(fa.run(r))(((e) => f(e).run(r)))))
    def raiseError[A](e : E) : Kleisli[F, R, A] =
      Kleisli.kleisli[F, R, A](((_) => F.raiseError(e)))
  }

  private trait KleisliContravariant[F[_], X]  extends Contravariant[ Kleisli[ F
  , *
  , X ] ] {
    def contramap[A, B](fa : Kleisli[F, A, X])(f : (B) => A) =
      ((fa) local f)
  }
  ...
  private trait KleisliStrong[F[_]]  extends Strong[Kleisli[F, *, *]] {
    ...
    override def second[A, B, C](f : Kleisli[F, A, B]) : Kleisli[ F
    , (C, A)
    , (C, B) ] =
      Kleisli({   case (c, a) =>   F.map(f.run(a))(((b : B) => (c, b))) })
    override def mapfst[A, B, C](fa : Kleisli[F, A, B])(f : (C) => A) =
      ((fa) local f)
    override def mapsnd[A, B, C](fa : Kleisli[F, A, B])(f : (B) => C) =
      ((fa) map f)
  }

  private trait KleisliProChoice[F[_]]  extends ProChoice[Kleisli[F, *, *]]
  ...
  private trait KleisliCompose[F[_]]  extends Compose[Kleisli[F, *, *]] {
    ...
    , ab : Kleisli[F, A, B] ) : Kleisli[F, A, C] =
      ((ab) >=> bc)
  }

  private trait KleisliArrow[F[_]]  extends Arrow[Kleisli[F, *, *]]
   with Choice[Kleisli[F, *, *]]
   ...
   with KleisliProChoice[F] {
    ...
    override def split[A, B, C, D]( f : Kleisli[F, A, B]
  ...
  private trait KleisliSemigroup[F[_], A, B]  extends Semigroup[ Kleisli[ F
  , A
  , B ] ] {
  ...
  private trait KleisliMonoid[F[_], A, B]  extends Monoid[Kleisli[F, A, B]]
  ...
  private trait KleisliPlusEmpty[F[_], A]  extends PlusEmpty[Kleisli[F, A, *]]
