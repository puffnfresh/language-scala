package scalaz {
  ...
  final case class WriterT[W, F[_], A] (run : F[(W, A)]) { self =>
    ...
    def ap[B](f : => WriterT[W, F, (A) => B])(implicit F : Apply[F], W : Semigroup[W]) : WriterT[W, F, B] =
      writerT( {
        F.apply2(f.run, run)( {
          case ((w1, fab), (w2, a)) =>
    ...
    def rwst[R, S](implicit F : Functor[F]) : ReaderWriterStateT[R, W, S, F, A] =
  ...
  sealed abstract class WriterTInstances4  extends WriterTInstance5 {
    ...
    implicit def writerTEqual[F[_], W, A](implicit E : Equal[F[(W, A)]]) : Equal[WriterT[W, F, A]] =
  ...
  trait WriterTFunctions  {
    def writerT[F[_], W, A](v : F[(W, A)]) : WriterT[W, F, A] =
      WriterT(v)
    def writerTU[FAB, AB, A0, B0](fab : FAB)( implicit u1 : Unapply[Functor, FAB] {type A = AB}
  ...
  private trait WriterTDivisible[F[_], W]  extends Divisible[WriterT[W, F, *]] {
    ...
    override def divide2[A1, A2, Z](a1 : => WriterT[W, F, A1], a2 : => WriterT[W, F, A2])(f : (Z) => (A1, A2)) : WriterT[W, F, Z] =
      WriterT(F.divide2[(W, A1), (W, A2), (W, Z)](a1.run, a2.run)((z : (W, Z)) => (z._1, f(z._2)) match {   case (w, (l, r)) =>   ((w, l), (w, r)) }))
  }
  ...
  private trait WriterTDecidable[F[_], W]  extends Decidable[WriterT[W, F, *]] with WriterTDivisible[F, W] {
    ...
    override def choose2[Z, A1, A2](a1 : => WriterT[W, F, A1], a2 : => WriterT[W, F, A2])(f : (Z) => A1 \/ A2) : WriterT[W, F, Z] =
      WriterT(F.choose2[(W, Z), (W, A1), (W, A2)](a1.run, a2.run)((z : (W, Z)) => f(z._2).fold((l) => -\/((z._1, l)), (r) => \/-((z._1, r)))))
  }
  ...
  private trait WriterTApply[F[_], W]  extends Apply[WriterT[W, F, *]] with WriterTFunctor[F, W] {
    ...
    override def ap[A, B](fa : => WriterT[W, F, A])(f : => WriterT[W, F, (A) => B]) =
      fa ap f
  }
  ...
  private trait WriterTBind[F[_], W]  extends Bind[WriterT[W, F, *]] with WriterTApply[F, W] {
    ...
    override final def bind[A, B](fa : WriterT[W, F, A])(f : (A) => WriterT[W, F, B]) =
      fa flatMap f
  }
  ...
  private trait WriterTFoldable[F[_], W]  extends Foldable.FromFoldr[WriterT[W, F, *]] {
    ...
    override def foldRight[A, B](fa : WriterT[W, F, A], z : => B)(f : (A, => B) => B) =
      fa.foldRight(z)(f)
  }

  private trait WriterTTraverse[F[_], W]  extends Traverse[WriterT[W, F, *]] with WriterTFoldable[F, W] {
    implicit def F: Traverse[F]
    def traverseImpl[G[_]: Applicative, A, B](fa : WriterT[W, F, A])(f : (A) => G[B]) =
      fa traverse f
  }
